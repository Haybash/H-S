  ...
  while(1) 
{
    TaskInput();
    my_Loop(); 
    led_Pattern(); // led display patterns  
}
  return 0;
}
...
/**
  * @brief  On board leds light up in different patterns
  * @param  None
  * @retval None
  */
void led_Pattern(void)
...
while(1)
 {
 if(myTickCount - myTimer > 2000) 
 {
   for(i = 0; i < 3; i++)
    {
       BSP_LED_Toggle(LEDs[i]);    // Togglrs led 0 t0 2 every 2 secs
       myTimer = myTickCount;
    }    
       count++;                    // increment count
 }
 if(myTickCount - myTimer1 > 4000)
 {
    for(i = 5; i < numLEDs; i++)
     {
       BSP_LED_Toggle(LEDs[i]);    // Toggles led 6 t0 9 every 4 secs
       myTimer1 = myTickCount;
     }
	count++;    		   // increment count
    }
 if(count > 10) 
...
void CmdAdd(int mode)
{
  ...
  if(rc) {
    printf("Missing first value\n");
    return;
  }
    
  rc = fetch_uint32_arg(&B);
  if(rc) {
    printf("Missing second value\n");
    return;
  }
     sum = A + B;
  printf("Addition returns:\n"
	 " SUM IS EQUAL TO : %d\n", (int)sum);
} 

ADD_CMD("+",CmdAdd,"<first_value> <second_value>")
